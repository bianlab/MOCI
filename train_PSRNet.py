...
parser.add_argument("--epoch_sam_num", type=int, default=2000, help="per_epoch_iteration")
parser.add_argument("--init_lr", type=float, default=4e-4, help="initial learning rate")
parser.add_argument("--gpu_id", type=str, default='2', help='select gpu')


parser.add_argument("--pretrained_model_path", type=str, default='/ssd/wzh/home_ssd/1_SpecPol/1_SpecPolCode_Submit/exp/mask_0108_pols_all_pols_0421_train256_alternate_freq2/net_192epoch.pth', help='path log files')
...
parser.add_argument("--output_folder", type=str, default='./exp/Polarization_PSRNet/mask_0108_pols_all_pols_0530_sun_light/', help='output path')
...
parser.add_argument("--train_data_path_spec", type=str, default="/ssd/wzh/Dataset_HSI/HSI_Sun_Light_ALL/Train_Patch_650/", help='path datasets')
parser.add_argument("--valid_data_path_spec", type=str, default="/ssd/wzh/Dataset_HSI/HSI_Sun_Light_ALL/Valid_Patch_650/", help='path datasets')
...
def main():
    ...
    val_loader_spec = DataLoader(dataset=val_data_spec, batch_size=opt.batch_size, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    


    train_loader_hsipol = DataLoader(dataset=train_data_hsipol, batch_size=1, shuffle=True, num_workers=8,
                pin_memory=True, drop_last=True)
    val_loader_hsipol = DataLoader(dataset=val_data_hsipol, batch_size=1, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    

    train_spec_iter = cycle(train_loader_spec)
    train_hsipol_iter = cycle(train_loader_hsipol)



    alternate_freq = 10  # 每 10 个批次切换一次数据集
    spec_train_steps = 0
    ...
    while iteration < total_iteration:
        ...
        if iteration % (2 * alternate_freq) < alternate_freq:
            ...
            mask_patch = data_processing.get_random_mask_patches_pol(mask=mask, image_size=opt.image_size, patch_size=opt.train_patch_size, batch_size=opt.batch_size)
            #Generate the measurements using traning HSIs and selected sub-pattern

            inputs_HSIs, targets_HSIs = data_processing.get_mos_hsi(hsi=HSIs, mask=mask_patch, sigma=opt.sigma, mos_size=opt.train_patch_size[0], hsi_input_size=opt.train_patch_size[0], hsi_target_size=opt.train_patch_size[0])
            ...
            mask_patch = data_processing.get_random_mask_patches_pol_together(mask=mask, image_size=opt.image_size, patch_size=opt.train_patch_size, batch_size=1)
            #Generate the measurements using traning HSIs and selected sub-pattern

            inputs_HSIs, targets_HSIs = data_processing.get_mos_hsi_Norm_pols_togethor(hsi=HSIPOLs, mask=mask_patch, sigma=opt.sigma, mos_size=opt.train_patch_size[0], hsi_input_size=opt.train_patch_size[0], hsi_target_size=opt.train_patch_size[0])
